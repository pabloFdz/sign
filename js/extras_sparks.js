const newParticlesPerFrame = 600;
...
class TextSparks
{
    ...
    buildTextMask(texts) {
        ...
        const font = (size) => {
            return `bold ${size}px ${window.getComputedStyle( body, null ).getPropertyValue( "font-family" )}`;
        };
    ...
    createSparks() {
        sparksObject = new TextSparks();
        sparksObject.run();
    }
    destroySparks() {
        sparksObject.drawCB = null;
    }
}
let sparksObject = new TextSparks();